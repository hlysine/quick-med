...
function Treatment() {
  const data = useLoaderData({ from: '/managements/$key' });
  const params = useParams({ from: '/managements/$key' });
  useTextFragment();
  return (
    <div className="flex-1 p-4 overflow-y-auto self-center w-full max-w-250">
      <AddToBookmark
        link={`/managements/${params.key}`}
...
export const Route = createFileRoute('/managements/$key')({
  component: Treatment,
  loader: async ({ params }) => {
    return (
      (await import(`../../content/managements/${params.key}.md?raw`)) as {
        default: string;
      }
    ).default;
  },
